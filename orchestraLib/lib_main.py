...
def initialization_lib(in_p, out_p):
    global threshold, model, accidentals
    with open(f"{out_p}/Output.txt", "w") as text_file:
        text_file.write("Input Folder: %s" % in_p)
        text_file.write("Output Folder: %s" % out_p)
    ...
    for it in range(len(cutted)):
        ...
        for boundary in symbols_boundaries:
            ...
            for cutted_boundary in cutted_boundaries:
                ...
                if is_started is True and label != 'barline' and label != 'clef':
...
def main(in_path, out_path):
    """
    start_lib_function
    :param in_path: in_image_path
    :param out_path: out_txt_path
    :return: Nothing
    """
    in_path = str(in_path)
    out_path = str(out_path)
    initialization_lib(in_path, out_path)
    try:
        os.mkdir(out_path)
    except OSError:  # as error
        pass

    list_of_images = os.listdir(in_path)
    for _, fn in enumerate(list_of_images):
        # Open the output text file #
        file_prefix = fn.split('.')[0]
        f = open(f"{out_path}/{file_prefix}.txt", "w")

        # Process each image separately #
        try:
            process_image(in_path, fn, f)
        except Exception as e:
            print(e)
            print(f'{in_path}-{fn} has been failed !!')
            pass
