    ...
    for col in range(width):
        if in_img.item(line_start, col) == 0 or in_img.item(line_end, col) == 0:
            ...
            # If current staff can be extended up, then extend #
                ...
                if (col > 0 and in_img.item(line_end + 1, col - 1) != 0) and (
                        col < width - 1 and in_img.item(line_end + 1, col + 1) != 0):
            ...
            # If current staff can be extended down, then extend #
                ...
                if (col > 0 and in_img.item(line_start - 1, col - 1) != 0) and (
                        col < width - 1 and in_img.item(line_start - 1, col + 1) != 0):
                    thick = line_thickness + 1
                    if thick < 1:
                        thick = 1
                    for j in range(int(thick)):
...
def segmentation(height_before, in_img):
    ...
    for c in cnts:
        ...
        if threshold_min_area < area < threshold_max_area:
